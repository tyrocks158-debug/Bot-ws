...
class System:
    ...
    __slots__ = ['sid', 'name', 'url', 'keys', 'devices', 'numbers', 'stats']
...
# ── UI HELPERS ──────────────────────────────────────────────────────────────
def systems_text() -> str:
    lines = [f"\U0001F5C2 <b>Systems</b> ({len(SYSTEMS)})\n{DIV}"]
    for i, s in enumerate(system_list(), 1):
        st = s.stats
        lines.append(
            f"<b>{i}. {esc(s.name)}</b>\n"
            f"   \U0001F4BB {st['online']} online / {st['devices']} dev\n"
            f"   \U0001F4F1 {st['numbers']} numbers\n"
            f"   <code>{esc(s.url)}</code>")
    return "\n".join(lines)
...
def users_text() -> str:
    ...
    for uid, e in _access.items():
        ...
        lines.append(
            f"\U0001F464 <b>{esc(e.get('name') or 'User')}</b> (<code>{uid}</code>)\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin and uid in _admin_state:
        ...
        if state == "approve_sys":
            ...
            sys_obj = system_by_index(idx - 1) if idx > 0 else (system_list()[0] if SYSTEMS else None)
    ...
    if text in (BTN_GET, BTN_NEWNUM):
        ...
        if not num:
            await msg.reply_text(
                ...
                f"Try again later.", parse_mode="HTML", reply_markup=back_keyboard())
            return
