#!/usr/bin/env python3
"""
WP / Telegram Number Seller Bot — v9.17 (Railway-proof DB + Remove Stock)
======================================================

WHAT'S NEW IN v9.17
------------------------------------------------------------
1. DATABASE NEVER LOST ON RAILWAY (reset / crash / redeploy)
   - DB location: env var DATABASE_PATH / DB_PATH first, then
     /data/bot.db (Railway volume), then ./data/bot.db (local).
   - WAL is checkpointed (TRUNCATE) every 3 minutes AND on clean
     shutdown, so bot.db on the volume is always a complete file.
   - Automatic Telegram backups: every 12 h the bot sends its own
     .db to the SUPER ADMIN chat (ADMIN_ID) as a document.
     Restoring = just re-upload that file to the bot with
     /restorebackup (or drop it in the repo as data/bot.db).
   - On boot, if the DB is empty/missing and a Telegram backup
     exists, the newest backup is downloaded and restored
     automatically (no admin action needed).
   - Admin commands: /backup (send DB now), /restorebackup
     (reply to a .db file to restore it).
2. REMOVE STOCK (admin panel)
   - 🗑 Remove Stock button with two options:
     • Remove All Numbers — wipes every stock row (confirm step).
     • Remove Particular Number(s) — paste or upload a .txt with
       phone numbers (one per line); the bot deletes exactly those
       rows and reports found / not-found counts.
3. SUPPORT ID — support is now @usasupportcenter_1 everywhere,
   with an "Open Support" button that opens the chat directly.
...
DEPOSIT_GROUP_ID = _env_int("DEPOSIT_GROUP_ID", -1003990138543)
# Group where every successful purchase is logged (order history)
ORDER_HISTORY_GROUP_ID = _env_int("ORDER_HISTORY_GROUP_ID", -1004440829897)
...
# v9.17 RAILWAY-PROOF DB LOCATION (never lost on reset/crash/redeploy):
#   1) DATABASE_PATH / DB_PATH env var (Railway: point it to your Volume
#      mount, e.g. /data/bot.db)
#   2) /data/bot.db when a writable /data exists (Railway Volume)
#   3) ./data/bot.db locally (single-file deploy, as before)
_env_db = _env("DATABASE_PATH", "") or _env("DB_PATH", "")
if _env_db:
...
# v9.17: Telegram auto-backup (DB survives Railway reset/crash/redeploy)
TG_BACKUP_ENABLED = _env("TG_BACKUP_ENABLED", "1") not in ("0", "false", "no")
TG_BACKUP_INTERVAL_H = _env_int("TG_BACKUP_INTERVAL_H", 12)   # hours
WAL_CHECKPOINT_MIN = _env_int("WAL_CHECKPOINT_MIN", 3)        # minutes
# optional: manually set the Telegram file_id of a backup (worst case,
# no volume + wiped container). Normally NOT needed: the bot pins its
# newest backup message and reads the file_id from the pin at boot.
TG_BACKUP_FILE_ID = _env("TG_BACKUP_FILE_ID", "")
# tiny sidecar file (NOT inside the DB) remembering the newest Telegram
# backup file_id — survives even if the whole DB file is wiped
DB_LAST_BACKUP_FILE = DB_PATH + ".lastbackup"
...
SUPPORT_USERNAME = _env("SUPPORT_USERNAME", "@usasupportcenter_1")
...
BTN_REMOVE_STOCK = "\U0001F5D1 Remove Stock"
...
# v9.17 Remove Stock buttons
BTN_RM_ALL     = "\U0001F5D1 Remove All Numbers"
BTN_RM_ONE     = "\U0001F50D Remove Particular Number(s)"
BTN_RM_CONFIRM = "\u26A0\uFE0F Yes, Remove Everything"
BTN_RM_CANCEL  = "\u274C No, Keep It"
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # v9.17: Telegram backup ledger (which .db backups exist)
        await db.execute(
...
# --------------------------------------------------------------------------- #
#  v9.17  TELEGRAM BACKUP / RESTORE  (Railway reset/crash/deploy-proof DB)
# --------------------------------------------------------------------------- #
BACKUP_CAPTION = "🛡 DB BACKUP (v9.17) — reply to this file with /restorebackup to restore"


def _db_is_fresh() -> bool:
    """A 'fresh' DB has no users and no stock (or no tables at all) ->
    safe to overwrite from a backup (this is exactly the state after a
    Railway reset)."""
    try:
        import sqlite3 as _s
        con = _s.connect(DB_PATH, timeout=5)
        try:
            names = {r[0] for r in con.execute(
                "SELECT name FROM sqlite_master WHERE type='table'")}
            if "users" not in names:
                return True          # brand-new / never-initialized DB file
            users = con.execute("SELECT COUNT(*) FROM users").fetchone()[0]
            stock = con.execute("SELECT COUNT(*) FROM stock").fetchone()[0]
        finally:
            con.close()
        return users == 0 and stock == 0
    except Exception:
        return True                  # unreadable/missing -> restorable


def _checkpoint_db() -> None:
    """Fold the WAL back into bot.db so the file is always complete."""
    try:
        import sqlite3 as _s
        con = _s.connect(DB_PATH, timeout=10)
        try:
...
async def record_tg_backup(file_id: str, file_unique_id: str = None):
    # 1) remember it inside the DB ...
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            await db.execute(
                "INSERT INTO tg_backups(file_unique_id,file_id) VALUES(?,?)",
                (file_unique_id, file_id),
            )
            await db.commit()
    except Exception as e:
        logger.warning("record_tg_backup (db) failed: %s", e)
    # 2) ... AND in a tiny sidecar file next to the DB, so even a wiped
    #    volume can still find the newest backup file_id on next boot
    try:
        with open(DB_LAST_BACKUP_FILE, "w", encoding="utf-8") as fh:
            fh.write(str(file_id))
    except Exception as e:
        logger.warning("record_tg_backup (sidecar) failed: %s", e)


async def _send_db_backup(bot, chat_id: int, pin: bool = False,
                          caption: str = None):
    """Checkpoint + send bot.db to a chat; records the new file_id.
    pin=True (admin chat only) also pins the message, so the backup
    survives even a full container wipe (bot reads the pin at boot).
    Returns the Telegram file_id (or None on failure)."""
    try:
        _checkpoint_db()
        with open(DB_PATH, "rb") as fh:
            msg = await bot.send_document(
                chat_id=chat_id,
                document=InputFile(fh, filename="bot-backup.db"),
                caption=caption or BACKUP_CAPTION,
            )
        f = getattr(msg, "document", None)
        if f:
            try:
                await record_tg_backup(f.file_id, f.file_unique_id)
            except Exception as e:
                logger.warning("record_tg_backup failed: %s", e)
            if pin:
                # unpin old backups, pin the newest one
                try:
                ...
                try:
                    ...
                    logger.warning("pin backup failed: %s", e)
            return f.file_id
        return None
    except Exception as e:
        logger.warning("send_db_backup failed: %s", e)
        return None


async def _find_backup_file_id_from_pin(bot) -> str:
    """Read the newest backup file_id from the PINNED message in the
    super-admin chat (survives container/volume wipes — the pin lives
    on Telegram's servers)."""
    try:
        pm = await bot.get_chat(chat_id=ADMIN_ID).pinned_message
    except Exception:
        pm = None
    if pm is None:
        # older PTB versions: get_chat returns Chat without pinned_message
        try:
            chat = await bot.get_chat(ADMIN_ID)
            pm = getattr(chat, "pinned_message", None)
        except Exception:
            pm = None
    if pm is not None and getattr(pm, "document", None):
        d = pm.document
        if (d.file_name or "").lower().endswith((".db", ".sqlite", ".sqlite3")):
            return d.file_id
    return None


async def periodic_backup_job(context):
    """Every TG_BACKUP_INTERVAL_H: send the DB to the super admin and
    pin it (the pin is read at boot -> restore works even after a full
    container/volume wipe)."""
    app = context.application
    try:
        fid = await _send_db_backup(app.bot, ADMIN_ID, pin=True)
        if fid:
            logger.info("Auto DB backup sent+ pinned for admin %s",
                        ADMIN_ID)
    except Exception as e:
        logger.warning("periodic_backup_job failed: %s", e)


async def send_backup_now(update, context):
    """Admin command /backup: send the .db right now."""
    if not is_admin(update.effective_user.id):
        return
    fid = await _send_db_backup(context.bot, update.effective_chat.id)
    if not fid:
        await update.message.reply_text("⚠️ Backup failed — check logs.")
        return
    await update.message.reply_text(
        "✅ <b>BACKUP SENT</b>\n"
        f"{DIV}\n"
        "💾 File: <code>bot-backup.db</code>\n"
        "📌 Keep it safe — upload it to this chat and tap "
        "<b>Restore Backup</b> (or /restorebackup) to recover.",
        parse_mode=ParseMode.HTML)


async def restore_from_tg_file(update, context, doc):
    """Restore the DB from a Telegram .db document.
    Rules:
      - only admins, only .db / .sqlite files
      - if the live DB has data, a fresh safety backup is sent first
      - the file is applied only after validation (must open as SQLite
        and contain a users table)"""
    uid = update.effective_user.id
    if not is_admin(uid):
        return
    ...
    if not (name.endswith(".db") or name.endswith(".sqlite")
        ...
        await update.message.reply_text(
            "⚠️ <b>Not a database file.</b> Send the <code>bot-backup.db</code> "
            "you received from the bot.",
            parse_mode=ParseMode.HTML)
        return
    if (doc.file_size or 0) > 25_000_000:
        await update.message.reply_text("⚠️ File too big (max 25 MB).")
        return
    try:
    ...
    # validate: must be a SQLite DB with a users table
    import tempfile
    tmp_path = None
    try:
        with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as tf:
            ...
            tmp_path = tf.name
        import sqlite3 as _s
        con = _s.connect(tmp_path, timeout=5)
        try:
            tables = {r[0] for r in con.execute(
                "SELECT name FROM sqlite_master WHERE type='table'")}
            if "users" not in tables or "stock" not in tables:
                raise ValueError("missing users/stock tables")
        finally:
            con.close()
    except Exception as e:
        if tmp_path and os.path.exists(tmp_path):
            os.remove(tmp_path)
        await update.message.reply_text(
            f"❌ <b>Invalid backup file</b> ({html_escape(str(e))}). "
            "Use the bot-backup.db the bot sent you.",
            parse_mode=ParseMode.HTML)
        return

    # safety: if the live DB still has data, back it up BEFORE overwriting
    if not _db_is_fresh():
        await update.message.reply_text(
            "⏳ <b>Live DB has data</b> — sending a safety backup first…",
            parse_mode=ParseMode.HTML)
        await _send_db_backup(context.bot, update.effective_chat.id)

    try:
        _checkpoint_db()
        # remove DB + WAL/SHM side files, then drop the backup in place
    ...
    # migrate schema if the backup was made by an older version
    try:
        await init_db()
    except Exception as e:
        logger.warning("post-restore init_db: %s", e)
    await update.message.reply_text(
        "✅ <b>DATABASE RESTORED</b>\n"
        f"{DIV}\n"
        f"💾 <code>{html_escape(doc.file_name or 'bot-backup.db')}</code>\n"
        "♻️ Old data is back — tap /start to reload your balance/stock.",
        parse_mode=ParseMode.HTML)


async def try_boot_restore(bot) -> bool:
    """Called at boot: if the DB is fresh/empty (Railway reset wiped it)
    and a Telegram backup exists, download the newest one and restore."""
    if not _db_is_fresh():
        return False            # DB has data -> nothing to do
    if not TG_BACKUP_ENABLED:
        return False
    file_id = None
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            cur = await db.execute(
                "SELECT file_id FROM tg_backups ORDER BY id DESC LIMIT 1")
            row = await cur.fetchone()
            await cur.close()
        ...
        file_id = None
    if not file_id:
        # sidecar next to the DB survives a wiped volume too
        try:
            if os.path.exists(DB_LAST_BACKUP_FILE):
                with open(DB_LAST_BACKUP_FILE, encoding="utf-8") as fh:
                    file_id = fh.read().strip() or None
        except Exception:
            file_id = None
    if not file_id and TG_BACKUP_FILE_ID:
        file_id = TG_BACKUP_FILE_ID          # manual env override
    if not file_id:
        # last resort: the PINNED backup message in the admin chat
        # (lives on Telegram's servers — survives any container wipe)
        try:
            file_id = await _find_backup_file_id_from_pin(bot)
        except Exception:
            file_id = None
    if not file_id:
        return False
    logger.info("Fresh DB detected — restoring from Telegram backup %s…",
                str(file_id)[:24])
    try:
        tg_file = await bot.get_file(file_id)
        raw = bytes(await tg_file.download_as_bytearray())
        import tempfile, sqlite3 as _s
        with tempfile.NamedTemporaryFile(suffix=".db", delete=False) as tf:
            tf.write(raw)
            tmp_path = tf.name
        con = _s.connect(tmp_path, timeout=5)
        ...
        _sh.copy2(tmp_path, DB_PATH)
        os.remove(tmp_path)
        await init_db()      # ensure schema of the current version
        logger.info("DB RESTORED from Telegram backup (users/stock back).")
        return True
    except Exception as e:
        logger.warning("boot restore failed: %s", e)
        return False
...
async def delete_all_stock() -> int:
    """v9.17: wipe EVERY stock row (available + sold + refunded).
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        try:
            # keep AUTOINCREMENT ids from colliding with old receipts
            await db.execute("DELETE FROM sqlite_sequence WHERE name='stock'")
        except Exception:
...
async def delete_stock_by_phone(phone: str) -> int:
    """v9.17: delete every stock row for one phone number.
    Returns how many rows were deleted (0 = not found)."""
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "DELETE FROM stock WHERE phone_number=?", (phone,))
        await db.commit()
        return cur.rowcount


async def delete_stock_by_phone_any(phone: str) -> int:
    """v9.17: delete a stock row no matter how the number was typed.
    Tries the exact +digits, then the +1 country-code variant (US numbers
    are often stored as +14133525885 but typed as 4133525885)."""
    digits = re.sub(r"\D", "", phone or "")
    if not (10 <= len(digits) <= 15):
        return 0
    candidates = ["+" + digits]
    if len(digits) == 10:                    # 4133525885 -> +14133525885
        candidates.append("+1" + digits)
    elif len(digits) == 11 and digits.startswith("1"):
        candidates.append("+" + digits[1:])  # 14133525885 -> +4133525885
    for cand in candidates:
        n = await delete_stock_by_phone(cand)
        if n > 0:
            return n
    return 0
    ...
    return _kb(
        [
            [KeyboardButton(BTN_ADD_STOCK, style=KeyboardButtonStyle.SUCCESS),
             KeyboardButton(BTN_REMOVE_STOCK, style=KeyboardButtonStyle.DANGER),
...
def admin_remove_stock_kb() -> InlineKeyboardMarkup:
    """v9.17 Remove Stock picker: all numbers vs particular numbers."""
    return _ikb(
        [
            [InlineKeyboardButton(BTN_RM_ALL, callback_data="adrm:all",
                                  style=KeyboardButtonStyle.DANGER)],
            [InlineKeyboardButton(BTN_RM_ONE, callback_data="adrm:one",
                                  style=KeyboardButtonStyle.PRIMARY)],
            [InlineKeyboardButton("\u2b05\ufe0f Back", callback_data="nav:back",
                                  style=KeyboardButtonStyle.DANGER)],
        ]
...
def admin_remove_all_confirm_kb() -> InlineKeyboardMarkup:
    """Double-confirm keyboard for the destructive 'remove all'."""
    return _ikb(
        [
            [InlineKeyboardButton(BTN_RM_CONFIRM,
                                  callback_data="adrm:allgo",
                                  style=KeyboardButtonStyle.DANGER)],
            [InlineKeyboardButton(BTN_RM_CANCEL, callback_data="adrm:menu",
                                  style=KeyboardButtonStyle.SUCCESS)],
        ]
    )
...
def support_text() -> str:
    return (
        ...
        f"{DIV}\n"
        f"\U0001F468\u200D\U0001F4BB Support: {SUPPORT_USERNAME}\n"
...
def support_url() -> str:
    """v9.17: direct t.me link for the support id."""
    uname = (SUPPORT_USERNAME or "").strip().lstrip("@")
    return f"https://t.me/{uname}" if uname else "https://t.me/"


def support_kb() -> InlineKeyboardMarkup:
    """v9.17: Support screen — direct chat button + Main Menu."""
    rows = [[InlineKeyboardButton(
        "\U0001F4AC Open Support", url=support_url(),
        style=KeyboardButtonStyle.SUCCESS)]]
    rows.append(_inline_nav_row())
    return _ikb(rows)
...
async def show_support(update, context):
    await clear_user_state(update.effective_user.id)
    await _send_tracked(update, context, support_text(), support_kb())
...
async def show_admin_remove_stock(update, context):
    """v9.17: 🗑 REMOVE STOCK picker — all numbers or particular ones."""
    await set_user_state(update.effective_user.id, "admin_remove_pick")
    counts, total = await get_stock_counts()
    await _send_tracked(update, context,
                        "\U0001F5D1 <b>REMOVE STOCK</b>\n\n"
                        f"{DIV}\n"
                        f"\U0001F4E6 <b>Numbers in stock:</b> {total}\n"
                        f"{DIV}\n"
                        "\U0001F447 <b>Select what to remove</b>",
                        admin_remove_stock_kb())


async def prompt_admin_remove_phone(update, context):
    """v9.17: remove particular number(s) — paste or upload a .txt."""
    await set_user_state(update.effective_user.id, "admin_remove_phone")
    await _send_tracked(update, context,
                        "\U0001F50D <b>REMOVE PARTICULAR NUMBER(S)</b>\n\n"
                        f"{DIV}\n"
                        "\U0001F4F1 <b>Send number(s) — one per line:</b>\n"
                        "<code>+14133525884</code>\n"
                        "<code>+14133525885</code>\n\n"
                        "\U0001F4C4 <b>Or upload a .txt file</b> with the "
                        "numbers.\n"
                        "\U0001F527 <i>Missing +, spaces, ( ) - and "
                        "country formats are fixed automatically.</i>\n"
                        f"{DIV}\n"
                        "\U0001F4B5 <b>Result:</b> only the matching stock "
                        "rows are deleted.\n"
                        "\u274C <b>Cancel</b> to stop.",
                        admin_input_kb())


async def process_remove_stock_numbers(update, context, uid, text):
    """v9.17: parse pasted/uploaded numbers and delete those stock rows.
    Accepts bare numbers, formatted numbers AND full stock lines
    (\"+1xxx | http://otp-link\") — the URL is ignored, only the phone
    matters."""
    removed = not_found = bad = 0
    missing = []
    for line in (text or "").splitlines():
        ...
        if err or not res[0]:
            bad += 1
            continue
        n = await delete_stock_by_phone_any(res[0])
        if n > 0:
            removed += n
        else:
            not_found += 1
            missing.append(res[0])

    await clear_user_state(uid)
    lines = ["\U0001F5D1 <b>STOCK REMOVED</b>\n", DIV,
             f"\u2705 <b>Rows deleted:</b> {removed}"]
    if not_found:
        lines.append(f"\u274C <b>Not found:</b> {not_found}")
    if bad:
        lines.append(f"\u26A0\uFE0F <b>Bad lines skipped:</b> {bad}")
    if missing:
        lines.append("\n\u274C <b>Not in stock:</b>")
        for p in missing[:15]:
    ...
    counts, total = await get_stock_counts()
    lines += ["\n" + DIV,
              f"\U0001F4E6 <b>Remaining in stock:</b> {total}",
              DIV]
...
async def go_back(update, context, state, sdata):
    ...
    if state in ("admin_remove_all", "admin_remove_pick"):
        return await show_admin_remove_stock(update, context)
    if state == "admin_remove_phone":
        return await show_admin_remove_stock(update, context)
...
async def reshow_state(update, context, state, sdata):
    ...
    if state == "admin_add_type":
        return await show_admin_add_type(update, context)
    if state == "admin_remove_pick":
        return await show_admin_remove_stock(update, context)
    if state == "admin_remove_phone":
        return await prompt_admin_remove_phone(update, context)
    if state == "admin_add_country":
...
# --------------------------------------------------------------------------- #
async def restorebackup_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin: /restorebackup — reply to a bot-backup.db document, or just
    send the .db file to the bot (it restores automatically)."""
    uid = update.effective_user.id
    if not is_admin(uid):
    ...
    if replied:
        return await restore_from_tg_file(update, context, replied)
    await update.message.reply_text(
        "♻️ <b>RESTORE BACKUP</b>\n"
        f"{DIV}\n"
        "📤 Now <b>send the <code>bot-backup.db</code> file</b> to this "
        "chat (upload it as a document).\n"
        "🛡 The current DB is backed up automatically before it is "
        "replaced.",
        parse_mode=ParseMode.HTML)


async def wal_checkpoint_job(context):
    """Fold the WAL back into bot.db so the file on disk is always a
    complete database (no data stuck in -wal when Railway redeploys)."""
    try:
        _checkpoint_db()
    except Exception as e:
        logger.warning("wal_checkpoint_job failed: %s", e)
...
async def admin_flow_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if grp == "adrm":
        # v9.17 Remove Stock: menu / all (confirm) / one
        if act == "menu":
            return await show_admin_remove_stock(update, context)
        if act == "all":
            # destructive -> show a double-confirm card first
            counts, total = await get_stock_counts()
            wa = sum(counts.get("whatsapp", {}).values())
            tg = sum(counts.get("telegram", {}).values())
            await set_user_state(update.effective_user.id,
                                 "admin_remove_all")
            try:
                await q.edit_message_text(
                    "\u26A0\uFE0F <b>REMOVE ALL NUMBERS?</b>\n\n"
                    f"{DIV}\n"
                    "This deletes <b>EVERY stock number</b> in the bot:\n"
                    f"\U0001F7E2 WhatsApp available: <b>{wa}</b>\n"
                    f"\U0001F535 Telegram available: <b>{tg}</b>\n"
                    f"\U0001F4E6 Total available: <b>{total}</b>\n"
                    f"{DIV}\n"
                    "\U0001F6A8 Sold-number history in orders/receipts "
                    "stays, but the stock list will be <b>empty</b>.\n"
                    "This cannot be undone.\n"
                    f"{DIV}\n"
                    "\U0001F447 <b>Are you sure?</b>",
                    parse_mode=ParseMode.HTML,
                    reply_markup=admin_remove_all_confirm_kb())
            except Exception:
                await _send_tracked(update, context,
                                    "\u26A0\uFE0F <b>REMOVE ALL NUMBERS?</b> "
                                    "\u2014 tap a button above.",
                                    admin_remove_all_confirm_kb())
            return
        if act == "allgo":
            n = await delete_all_stock()
            await clear_user_state(update.effective_user.id)
            await q.answer(
                f"\u2705 {n} number(s) removed", show_alert=False)
            try:
                await q.edit_message_text(
                    "\U0001F5D1 <b>ALL STOCK REMOVED</b>\n\n"
                    f"{DIV}\n"
                    f"\u2705 <b>Rows deleted:</b> {n}\n"
                    f"\U0001F4E6 <b>Remaining in stock:</b> 0\n"
                    f"{DIV}",
                    parse_mode=ParseMode.HTML)
            except Exception:
                pass
...
async def handle_state_text(update, context, state, sdata, text):
    ...
    if state == "admin_remove_phone":
        # v9.17: remove particular stock numbers (pasted or .txt content)
        if not re.search(r"\d", text or ""):
            await _send_tracked(update, context,
                                "\u274C <b>No numbers found.</b> Send them "
                                "one per line, e.g. <code>+14133525884</code>:",
                                admin_input_kb())
            return
        return await process_remove_stock_numbers(update, context, uid, text)
...
async def on_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # v9.17: admin sends a .db file -> DB restore flow
    doc = update.message.document
    if doc and is_admin(uid):
        _name = (doc.file_name or "").lower()
        if _name.endswith((".db", ".sqlite", ".sqlite3")):
            await update.message.reply_text(
                "\u23F3 <b>Restore file received</b> \u2014 validating\u2026",
                parse_mode=ParseMode.HTML,
            )
            return await restore_from_tg_file(update, context, doc)
    ...
    # v9.17 Admin: Remove Stock via .txt upload (numbers to delete)
    if state == "admin_remove_phone" and is_admin(uid):
        doc = update.message.document
        name = (doc.file_name or "").lower()
        mime = (doc.mime_type or "")
        ok_type = (name.endswith((".txt", ".text", ".csv"))
        ...
        if (doc.file_size or 0) > 2_000_000:
            await update.message.reply_text(
                "\u26A0\uFE0F <b>File too big (max 2 MB).</b>\n"
                "Split it or paste the numbers directly.",
                parse_mode=ParseMode.HTML,
            )
            return
        tg_file = await context.bot.get_file(doc.file_id)
        ...
        text = bytes(raw).decode("utf-8", errors="replace")
        await update.message.reply_text(
            "\u23F3 <b>Removing numbers from file...</b>",
            parse_mode=ParseMode.HTML,
        )
        await process_remove_stock_numbers(update, context, uid, text)
        return
...
async def post_init(app: Application):
    await init_db()
    # v9.17: Railway wiped the DB? auto-restore from the newest
    # Telegram backup before anything else runs
    try:
        if TG_BACKUP_ENABLED:
            await try_boot_restore(app.bot)
    except Exception as e:
        logger.warning("boot restore failed: %s", e)
    try:
        await load_admins()
    ...
    try:
        await app.bot.set_my_commands([
            ...
            ("admin", "Admin panel"),
            ("help", "Help"),
            ("backup", "Admin: DB backup file"),
            ("restorebackup", "Admin: restore DB file"),
        ])
    except Exception as e:
    ...
    logger.info("DB: %s (backups every %sh to admin %s)",
                DB_PATH, TG_BACKUP_INTERVAL_H, ADMIN_ID)

    # v9.17: PTB was installed without the [job-queue] extra? run the
    # same jobs as plain asyncio loops so backups NEVER stop happening.
    if not getattr(app, "job_queue", None):
        class _JobCtx:  # tiny stub that looks like a job_queue context
            def __init__(self, application):
                self.application = application

        async def _fallback_backup_loop():
            # first backup 5 min after boot, then every N hours
            await asyncio.sleep(300)
            while True:
                try:
                    await periodic_backup_job(_JobCtx(app))
                except asyncio.CancelledError:
                    raise
                except Exception as e:
                    logger.warning("fallback backup loop: %s", e)
                await asyncio.sleep(max(1, TG_BACKUP_INTERVAL_H) * 3600)

        async def _fallback_checkpoint_loop():
            while True:
                await asyncio.sleep(max(1, WAL_CHECKPOINT_MIN) * 60)
                try:
                    _checkpoint_db()
                except asyncio.CancelledError:
                    raise
                except Exception as e:
                    logger.warning("fallback wal_checkpoint: %s", e)

        _jobs = set()
        if TG_BACKUP_ENABLED and TG_BACKUP_INTERVAL_H > 0:
            _jobs.add(asyncio.create_task(_fallback_backup_loop()))
        _jobs.add(asyncio.create_task(_fallback_checkpoint_loop()))
        app.bot_data.setdefault("v917_fallback_jobs", set()).update(_jobs)
        logger.info("Fallback jobs started via asyncio "
                    "(backup every %sh, checkpoint every %s min)",
                    TG_BACKUP_INTERVAL_H, WAL_CHECKPOINT_MIN)
...
def build_app() -> Application:
    ...
    app.add_handler(CommandHandler("backup", send_backup_now))
    ...
    app.add_handler(CallbackQueryHandler(admin_flow_callback,
                                         pattern=r"^(adst|adstc|adrt|adbn|admg|adrm|admuser):"))
    app.add_handler(CallbackQueryHandler(nav_callback, pattern=r"^nav:"))
    app.add_handler(CallbackQueryHandler(fallback_callback))

    # v9.17 jobs: Telegram DB backup + WAL checkpoint keep bot.db safe
    # (job_queue needs the [job-queue] extra; when it is missing,
    # post_init starts plain asyncio fallback loops instead)
    if getattr(app, "job_queue", None):
        app.job_queue.run_repeating(
            wal_checkpoint_job, interval=max(1, WAL_CHECKPOINT_MIN) * 60,
            first=max(1, WAL_CHECKPOINT_MIN) * 60, name="wal_checkpoint")
        if TG_BACKUP_ENABLED and TG_BACKUP_INTERVAL_H > 0:
            app.job_queue.run_repeating(
                periodic_backup_job, interval=TG_BACKUP_INTERVAL_H * 3600,
                first=300, name="tg_db_backup")  # first backup 5 min after boot
        logger.info("Jobs scheduled via job_queue (checkpoint %s min, "
                    "backup every %s h)", WAL_CHECKPOINT_MIN,
                    TG_BACKUP_INTERVAL_H)
    else:
        logger.info("job_queue NOT installed — asyncio fallback jobs "
                    "will start in post_init")

    app.add_error_handler(on_error)
    return app


def _final_backup_on_shutdown() -> None:
    """v9.17: on a CLEAN shutdown (Railway redeploy sends SIGTERM),
    push one last DB backup to the admin chat so nothing written in the
    final minutes is lost. Uses its own Bot instance + a hard timeout so
    it can never hang the exit."""
    if not TG_BACKUP_ENABLED:
        return
    ...
    async def _go():
        ...
        try:
            await _send_db_backup(
                bot, ADMIN_ID, pin=True,
                caption="🛡 DB BACKUP (final, on shutdown) — "
                        "auto-restore happens on next boot")
        finally:
            try:
                await bot.shutdown()
            except Exception:
    ...
    try:
        ...
        logger.warning("final shutdown backup failed: %s", e)
...
def main():
    ...
    while True:
        attempt += 1
        try:
            logger.info("Starting bot v9.17 (Railway-proof DB: WAL "
                        "checkpoints + Telegram auto-backup/restore, "
                        "Remove Stock admin tool)... attempt %s", attempt)
            app = build_app()
            app.run_polling(allowed_updates=Update.ALL_TYPES)
            logger.info("Clean shutdown - supervisor exiting.")
            # v9.17: last-chance backup before the container dies
            _final_backup_on_shutdown()
            break
